...
#tile_plg_creator = plg_registry.get_plugin_creator("BatchTilePlugin_TRT", "1", "")
#if tile_plg_creator is None:
#    print("tile_plg_creator is None!")
#else:
#    print("get tile_plg_creator success!")

def add_constant(network, value, tensor=None):
    if tensor is None:
        shape = (1,)
    ...
    scalar = np.ones(shape, dtype=dtype) * value
    return network.add_constant(shape, scalar).get_output(0)

class BertConfig:
    def __init__(self, bert_config_path, use_fp16, use_int8, use_qat, interleaved, timing_cache, use_sparsity, use_megatron):
        with open(bert_config_path, "r") as f:
            data = json.load(f)
            self.hidden_act = data["hidden_act"]
...
def skipln(prefix, config, init_dict, network, input_tensor, skip, is_last_skipln=False):
    ...
    # NOTE: 部分bert存在diff问题, 原因发现skiln后diff变大,所以这里用fp32来计算
    #dtype = config.get_trt_dtype()    # edit by fonsotang 3.15
    dtype = trt.float32
...
def transformer_layer_opt(prefix, config, init_dict, network, input_tensor, residual, mask_idx, cu_seqlens, max_seqlen):
    ...
    if config.hidden_act == "gelu":
        print("use gelu in transformer_layer_opt.")
        gelu_layer = add_gelu(network, mid_dense.get_output(0))
        intermediate_act = gelu_layer.get_output(0)
    elif config.hidden_act == "relu":
        print("use relu in transformer_layer_opt.")
        relu1 = network.add_activation(input=mid_dense.get_output(0), type=trt.ActivationType.RELU)
        intermediate_act = relu1.get_output(0)
    else:
        TRT_LOGGER.log(TRT_LOGGER.ERROR, "unsupported activation function {} Sec".format(config.hidden_act))
    set_tensor_name(intermediate_act, prefix, config.hidden_act)
...
def bert_model(config, init_dict, network, input_tensor, residual, mask_idx, cu_seqlens, max_seqlen, batch_size):
    """
...
def cls_output(prefix, config, init_dict, network, input_tensor, cu_seq_lens, batch_size, label_num=2):
    """
    Create the cls output
    对应tf代码: https://github.com/google-research/bert/blob/master/modeling.py
    """

    idims = input_tensor.shape
    print("in cls_output input_tensor.shape: {}".format(idims))
    assert len(idims) == 4
    #B, S, hidden_size, _, _ = idims
    S, hidden_size, _, _ = idims


    cu_seq_shape = network.add_shape(cu_seq_lens).get_output(0)
    constant_layer_out = network.add_constant(shape=(1, ), weights=np.array([1, ], dtype=np.int32)).get_output(0)
    cu_seq_real_shape = network.add_elementwise(cu_seq_shape, constant_layer_out, trt.tensorrt.ElementWiseOperation.SUB).get_output(0)
    slice_layer = network.add_slice(cu_seq_lens, start=(0,), shape=(1, ), stride=(1,))
    slice_layer.set_input(2, cu_seq_real_shape)
    slice_cu_seq_len = slice_layer.get_output(0)
    cls_layer = network.add_gather(input_tensor, slice_cu_seq_len, axis=0)
    print("cls_layer shape: {}".format(cls_layer.get_output(0).shape))

    shuffle_layer = network.add_shuffle(cls_layer.get_output(0))
    shuffle_layer.reshape_dims = [-1, hidden_size]

    input_tensor = shuffle_layer.get_output(0)
    meta_shape = (1,) * len(input_tensor.shape)
    norm = network.add_unary(input_tensor, trt.UnaryOperation.ABS).get_output(0)
    norm = network.add_elementwise(norm, add_constant(network, 2., meta_shape), trt.ElementWiseOperation.POW).get_output(0)
    norm = network.add_reduce(norm, trt.ReduceOperation.SUM, 1<<1, keep_dims=True).get_output(0)
    norm = network.add_elementwise(norm, add_constant(network, 0.5, meta_shape), trt.ElementWiseOperation.POW).get_output(0)
    norm = network.add_elementwise(norm, add_constant(network, 1e-5, meta_shape), trt.ElementWiseOperation.MAX).get_output(0)
    output_layer = network.add_elementwise(input_tensor, norm, trt.ElementWiseOperation.DIV)

    set_output_name(output_layer, "", "embedding")
    print("output shape:",output_layer.get_output(0).shape)
...
def emb_layernorm(builder, network, config, weights_dict, builder_config, max_sequence_length, max_batch_size):
    ...
    profile.set_shape("cu_seqlens", min=cu_seqlens_min_shape, opt=(max_batch_size+1,), max=(max_batch_size+1,))
...
def build_engine(batch_size, workspace_size, sequence_length, config, weights_dict, squad_json, vocab_file, calibrationCacheFile, calib_num):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(explicit_batch_flag) as network, builder.create_builder_config() as builder_config:
        ...
        #builder_config.flags = builder_config.flags | 1<<int(trt.BuilderFlag.STRICT_TYPES) # TODO debug 精度问题
        ...
        bert_out = bert_model(config, weights_dict, network, embeddings, residual, mask_idx, cu_seqlens, max_seqlen, batch_size)
        print("bert_out shape: {}".format(bert_out.shape))
        #network.mark_output(bert_out)

        label_num = 2   # 对于分类任务,输出的分类数
        cls_prob = cls_output("cls_", config, weights_dict, network, bert_out, cu_seqlens, batch_size, label_num)
        print("cls_prob shape: {}".format(cls_prob.get_output(0).shape))
        network.mark_output(cls_prob.get_output(0))
