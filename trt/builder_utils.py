...
def load_tf_weights(inputbase, config):
    ...
    try:
        ...
        if "adam" not in key and "global_step" not in key and "good_steps" not in key \
            and "current_loss_scale" not in key and "w_loss" not in key and "AdamWeightDecayOptimizer" not in key]
        ...
        for pn in param_names:
            print("{}, {}".format(pn, reader.get_tensor(pn).shape))
        print("-"*50)
    ...
    if toks[0] == 'bert': #embeddings or encoder
        if toks[1] == 'encoder': #transformer
            ...
            if toks[-2] == 'layernorm': #bias->beta, weight->gamma
                ...
                toks[-1] = 'bias' if toks[-1] == 'bias' else 'kernel'
        ...
        return name
    elif 'classifier' in onnx_name:
        name = 'cls_seq_relationship_output_bias' if toks[-1] == 'bias' else 'cls_seq_relationship_output_weights'
        return name
    else:
...
def get_onnx_weight_dict(tensor_dict, config):
    ...
    for outname, tensor in tensor_dict.items():
        if "bert_pooler" in outname:
            print("=====",outname, tensor)
    ...
    for key, value in weights_dict.items():
        if key.find("amax") != -1:
            item = ("float32", type(value))
        else:
            item = (value.numpy().dtype, value.numpy().shape)
        print("{} {} {}".format(key, item[0], item[1]))
    print("-"*50)
    return weights_dict
...
def load_onnx_weights_and_quant(path, config):
    ...
    print("-------------- onnx weights begin --------------")
    for w in weights:
        #print("{} {} {}".format(w.name, w.dims, w.raw_data.size()))
        data = np.frombuffer(w.raw_data, np.float32)
        print("{} {} {}".format(w.name, w.dims, data.shape))
    print("-------------- onnx weights end --------------")
    tensor_dict = dict()
    for w in weights:
        if w.name == "bert.embeddings.position_ids":
            continue
        if w.name.split('_')[-1] == 'mask':
            tensor_dict[onnx_to_trt_name(w.name)] = np.frombuffer(w.raw_data, np.int8).reshape(w.dims)
        else:
            tensor_dict[onnx_to_trt_name(w.name)] = np.frombuffer(w.raw_data, np.float32).reshape(w.dims)
...
def load_pytorch_weights_and_quant(path, config):
    ...
    print("-------------- torch weights begin --------------")
    for name, val in state_dict.items():
        np_value =val.numpy() if val.numpy().size == 1 else val.numpy().shape
        print("{} {} {}".format(name, val.numpy().dtype, np_value))
    print("-------------- torch weights end --------------")
    
    
    #print("====raw:",state_dict.keys())
    new_dict = {}
    ...
    for name, val in state_dict.items():
        if name == "bert.embeddings.position_ids":
            continue
        tensor_dict[onnx_to_trt_name(name)] = val.numpy()

    print("====convert:",tensor_dict.keys())
    return get_onnx_weight_dict(tensor_dict, config)
