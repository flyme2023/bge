...
#from helpers.calibrator import BertCalibrator as BertCalibrator
#from helpers.custom_calibrator import BertCalibrator as BertCalibrator
...
def cls_output(prefix, config, init_dict, network, input_tensor, label_num=5):
    """ 
    ...
    # slice 做pooler, 对应 self.sequence_output[:, 0:1, :] 
    input_shape = network.add_shape(input_tensor).get_output(0)
    div_shape = network.add_constant(shape=(5,), weights=np.array([S, 1, 1, 1, 1], dtype=np.int32)).get_output(0)
    real_shape = network.add_elementwise(input_shape, div_shape, trt.tensorrt.ElementWiseOperation.DIV).get_output(0)
    slice_layer = network.add_slice(input_tensor, start=(0, 0, 0, 0, 0), shape=(1, B, hidden_size, 1, 1), stride=(1, 1, 1, 1, 1))
    slice_layer.set_input(2, real_shape)
    
    # 做pooler的dense layer
    p_w = init_dict["bert_pooler_dense_kernel"]
    p_b = init_dict["bert_pooler_dense_bias"]
    pool_output = network.add_fully_connected(slice_layer.get_output(0), hidden_size, p_w, p_b)
    ...
    print("tanh_output shape: {}".format(tanh_output.shape))

    #squeeze_layer = network.add_shuffle(tanh_output)
    #squeeze_layer.reshape_dims = [B, hidden_size]

    #OUT = network.add_shuffle(squeeze_layer.get_output(0))
    OUT = network.add_shuffle(tanh_output)
    set_output_name(OUT, "", "embedding")
    return OUT
...
def squad_output(prefix, config, init_dict, network, input_tensor):
    ...
    slice_layer = network.add_slice(input_tensor, start=(0, 0, 0, 0, 0), shape=(1, B, hidden_size, 1, 1), stride=(1, 1, 1, 1, 1))
    slice_layer.set_input(2, real_shape)
    print("slice_layer output shape: {}".format(slice_layer.get_output(0).shape))

    shuffle_layer = network.add_shuffle(slice_layer.get_output(0))
    shuffle_layer.reshape_dims = [B, hidden_size]
...
def emb_layernorm(builder, network, config, weights_dict, builder_config, sequence_lengths, batch_sizes):
    ...
    input_ids = network.add_input(name="input_ids", dtype=trt.int32, shape=(-1, sequence_lengths[0]))
    segment_ids = network.add_input(name="segment_ids", dtype=trt.int32, shape=(-1, sequence_lengths[0]))
    input_mask = network.add_input(name="input_mask", dtype=trt.int32, shape=(-1, sequence_lengths[0]))
    ...
    opt_batch_size = 1 if (max_batch_size <= 1) else (max_batch_size // 2)
    profile = builder.create_optimization_profile()
    min_shape = (1, sequence_lengths[0])
    opt_shape = (opt_batch_size, sequence_lengths[0])
    max_shape = (max_batch_size, sequence_lengths[0])
    print("min_shape: {}, opt_shape: {}, max_shape: {}".format(min_shape, opt_shape, max_shape))
    ...
    '''
    profile = builder.create_optimization_profile()
    profile_batch = [1, 1, 10]
    min_shape = (profile_batch[0], sequence_lengths[0])
    opt_shape = (profile_batch[1], sequence_lengths[0])
    max_shape = (profile_batch[2], sequence_lengths[0])
    profile.set_shape("input_ids", min=min_shape, opt=opt_shape, max=max_shape)
    profile.set_shape("segment_ids", min=min_shape, opt=opt_shape, max=max_shape)
    profile.set_shape("input_mask", min=min_shape, opt=opt_shape, max=max_shape)
    builder_config.add_optimization_profile(profile)
    ...
    #mha_type = trt.PluginField("mha_type_id", np.array([trt.float32], np.int32), trt.PluginFieldType.INT32)
...
def build_engine(batch_sizes, workspace_size, sequence_lengths, config, weights_dict, squad_json, vocab_file, calibrationCacheFile, calib_num, verbose):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(explicit_batch_flag) as network, builder.create_builder_config() as builder_config:
        ...
        squad_logits_tensor = squad_logits.get_output(0)
        network.mark_output(squad_logits_tensor)
        print("=====squad_logits:",squad_logits_tensor.shape)
